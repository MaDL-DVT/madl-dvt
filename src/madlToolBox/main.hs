...
verifyNetwork network = do
    ...
    let options = Madl.Deadlock.Runner.defaultOptions{argRunMode = SmtOnly, argNuxmvOptions =  ReachabilityOptions { keepAigerModel = False, keepNuxmvModel = 0, reachabilityEngine = NUXMV IC3}}
    -- Cycle check        
    numC <- return 0;
