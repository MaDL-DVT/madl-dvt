...
-}
module Madl.BackInterpolation where
...
makeVars' net bound = let chans = getChannelIDs net
                          ...
                          irdyVars = map (\(x,y) -> "(declare-fun " ++ (show $ BVAR $ Irdy y x) ++ " () Bool)") [(s,c) | s <- [0..bound], c <- chans]
                          trdyVars = map (\(x,y) -> "(declare-fun " ++ (show $ BVAR $ Trdy y x) ++ " () Bool)") [(s,c) | s <- [0..bound], c <- chans]
                          dataVars = map (\(x,y) -> "(declare-fun " ++ (show $ IVAR $ Data y x) ++ " () Int) " ++ (makeColorAssertion' net y (colorSetToColors $ getColorSet net y) x ":interpolation-group ga")) [(bound,c) | c <- chans]
                          qcelVars = map (\(a,b,c) -> "(declare-fun " ++ (show $ IVAR $ QCell a b c) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QCell a b c) ++ " 0) :interpolation-group ga)) (assert (! (<= " ++ (show $ IVAR $ QCell a b c) ++ " " ++ (show ((BM.size (typeMap net))+1)) ++ ") :interpolation-group ga))") [(q,i,bound) | q <- qs, i <- [0..(getQueueSize net q)-1]]
                          qocVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ QOccupancy a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QOccupancy a b) ++ " 0) :interpolation-group ga)) (assert (! (<= " ++ (show $ IVAR $ QOccupancy a b) ++ " " ++ (show ((getQueueSize net a))) ++ ") :interpolation-group ga))") [(q,bound) | q <- qs]
                          stateVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Cur a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Cur a b) ++ " 0) :interpolation-group ga)) (assert (! (<= " ++ (show $ IVAR $ Cur a b) ++ " " ++ (show ((nrOfStates $ getComponent net a)-1)) ++ ") :interpolation-group ga))") [(p,bound) | p <- as]
                          transVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group ga)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " " ++ (show ((length $ transitions $ getComponent net a))) ++ ") :interpolation-group ga))") [(p,bound) | p <- as]
                          mrgVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group ga)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " 1) :interpolation-group ga))") [(p,bound) | p <- mrgs]
                          dataVars' = map (\(x,y) -> "(declare-fun " ++ (show $ IVAR $ Data y x) ++ " () Int) " ++ (makeColorAssertion' net y (colorSetToColors $ getColorSet net y) x ":interpolation-group gb")) [(s,c) | s <- [0..bound-2], c <- chans]
                          qcelVars' = map (\(a,b,c) -> "(declare-fun " ++ (show $ IVAR $ QCell a b c) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QCell a b c) ++ " 0) :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ QCell a b c) ++ " " ++ (show ((BM.size (typeMap net))+1)) ++ ") :interpolation-group gb))") [(q,i,s) | q <- qs, i <- [0..(getQueueSize net q)-1], s <- [0..bound-2]]
                          qocVars' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ QOccupancy a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QOccupancy a b) ++ " 0) :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ QOccupancy a b) ++ " " ++ (show ((getQueueSize net a))) ++ ") :interpolation-group gb))") [(q,s) | q <- qs, s <- [0..bound-2]]
                          stateVars' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Cur a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Cur a b) ++ " 0) :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Cur a b) ++ " " ++ (show ((nrOfStates $ getComponent net a)-1)) ++ ") :interpolation-group gb))") [(p,s) | p <- as, s <- [0..bound-2]]
                          transVars' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " " ++ (show ((length $ transitions $ getComponent net a))) ++ ") :interpolation-group gb))") [(p,s) | p <- as, s <- [0..bound-2]]
                          mrgVars' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " 1) :interpolation-group gb))") [(p,s) | p <- mrgs, s <- [0..bound-2]]
                          dataVars'' = map (\(x,y) -> "(declare-fun " ++ (show $ IVAR $ Data y x) ++ " () Int) " ++ (makeColorAssertion' net y (colorSetToColors $ getColorSet net y) x ":interpolation-group ga :interpolation-group gb")) [(bound-1,c) | c <- chans]
                          qcelVars'' = map (\(a,b,c) -> "(declare-fun " ++ (show $ IVAR $ QCell a b c) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QCell a b c) ++ " 0) :interpolation-group ga :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ QCell a b c) ++ " " ++ (show ((BM.size (typeMap net))+1)) ++ ") :interpolation-group ga :interpolation-group gb))") [(q,i,bound-1) | q <- qs, i <- [0..(getQueueSize net q)-1]]
                          qocVars'' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ QOccupancy a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ QOccupancy a b) ++ " 0) :interpolation-group ga :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ QOccupancy a b) ++ " " ++ (show ((getQueueSize net a))) ++ ") :interpolation-group ga :interpolation-group gb))") [(q,bound-1) | q <- qs]
                          stateVars'' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Cur a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Cur a b) ++ " 0) :interpolation-group ga :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Cur a b) ++ " " ++ (show ((nrOfStates $ getComponent net a)-1)) ++ ") :interpolation-group ga :interpolation-group gb))") [(p,bound-1) | p <- as]
                          transVars'' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group ga :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " " ++ (show ((length $ transitions $ getComponent net a))) ++ ") :interpolation-group ga :interpolation-group gb))") [(p,bound-1) | p <- as]
                          mrgVars'' = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (! (>= " ++ (show $ IVAR $ Sel a b) ++ " 0) :interpolation-group ga :interpolation-group gb)) (assert (! (<= " ++ (show $ IVAR $ Sel a b) ++ " 1) :interpolation-group ga :interpolation-group gb))") [(p,bound-1) | p <- mrgs]
                          gsvars = map (\x -> "(declare-fun global_state-" ++ (show x) ++ " () Bool)") [0..bound]
                          svars = map (\x -> "(declare-fun global_step-" ++ (show x) ++ " () Bool)") [1..bound]
                          ivars = map (\x -> "(declare-fun initial-" ++ (show x) ++ " () Bool)") [0..bound]
                          dlf = ["(declare-fun dlf () Bool)"]
...
makeVars'' net bound = let chans = getChannelIDs net
                           ...
                           as = getAllProcessIDs net
                           mrgs = getAllMergeIDs net
                           irdyVars = map (\(x,y) -> "(declare-fun " ++ (show $ BVAR $ Irdy y x) ++ " () Bool)") [(bound,c) | c <- chans]
                           trdyVars = map (\(x,y) -> "(declare-fun " ++ (show $ BVAR $ Trdy y x) ++ " () Bool)") [(bound,c) | c <- chans]
                           dataVars = map (\(x,y) -> "(declare-fun " ++ (show $ IVAR $ Data y x) ++ " () Int) " ++ (makeColorAssertion net y (colorSetToColors $ getColorSet net y) x)) [(bound,c) | c <- chans]
                           qcelVars = map (\(a,b,c) -> "(declare-fun " ++ (show $ IVAR $ QCell a b c) ++ " () Int) (assert (>= " ++ (show $ IVAR $ QCell a b c) ++ " 0)) (assert (<= " ++ (show $ IVAR $ QCell a b c) ++ " " ++ (show ((BM.size (typeMap net))+1)) ++ "))") [(q,i,bound) | q <- qs, i <- [0..(getQueueSize net q)-1]]
                           qocVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ QOccupancy a b) ++ " () Int) (assert (>= " ++ (show $ IVAR $ QOccupancy a b) ++ " 0)) (assert (<= " ++ (show $ IVAR $ QOccupancy a b) ++ " " ++ (show ((getQueueSize net a))) ++ "))") [(q,bound) | q <- qs]
                           stateVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Cur a b) ++ " () Int) (assert (>= " ++ (show $ IVAR $ Cur a b) ++ " 0)) (assert (<= " ++ (show $ IVAR $ Cur a b) ++ " " ++ (show ((nrOfStates $ getComponent net a)-1)) ++ "))") [(p,bound) | p <- as]
                           transVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (>= " ++ (show $ IVAR $ Sel a b) ++ " 0)) (assert (<= " ++ (show $ IVAR $ Sel a b) ++ " " ++ (show ((length $ transitions $ getComponent net a))) ++ "))") [(p,bound) | p <- as]
                           mrgVars = map (\(a,b) -> "(declare-fun " ++ (show $ IVAR $ Sel a b) ++ " () Int) (assert (>= " ++ (show $ IVAR $ Sel a b) ++ " 0)) (assert (<= " ++ (show $ IVAR $ Sel a b) ++ " 1))") [(p,bound) | p <- mrgs]
                           gsvars = map (\x -> "(declare-fun global_state-" ++ (show x) ++ " () Bool)") [bound]
                           svars = map (\x -> "(declare-fun global_step-" ++ (show x) ++ " () Bool)") [bound]
                           ivars = map (\x -> "(declare-fun initial-" ++ (show x) ++ " () Bool)") [bound]
                           dlf = ["(declare-fun dlf () Bool)"]
                       in foldr (\x y -> x ++ "\n" ++ y) "" (dlf ++ irdyVars ++ trdyVars ++ dataVars ++ qcelVars ++ qocVars ++ stateVars ++ transVars ++ mrgVars ++ gsvars ++ svars ++ ivars)
...
makeCheckInit :: Int -> String
makeCheckInit bound = let r = map (\x -> "initial-" ++ (show x)) [0..bound-1]
                          r' = foldr (\x y -> if y /= "" then x ++ " " ++ y else x ++ y) "" r
                      in if ((length r) > 1) then "(assert (or " ++ r' ++ "))" else "(assert " ++ r' ++ ")"


makeFormulas' :: ColoredNetwork -> Int -> String
makeFormulas' net bound = let state = foldr (\x y -> x ++ "\n\n" ++ y) "" (map (\x -> makeSignals'' net x " :interpolation-group gb") [0..bound-2])
                              state' = (makeSignals'' net (bound-1) " :interpolation-group ga :interpolation-group gb") ++ "\n\n"
                              state'' = (makeSignals'' net bound " :interpolation-group ga") ++ "\n\n"
                              step = foldr (\x y -> x ++ "\n\n" ++ y) "" (map (\x -> makeSteps'' net x " :interpolation-group gb") [1..bound-1])
                              --step' = (makeSteps'' net (bound-1) " :interpolation-group ga :interpolation-group gb") ++ "\n\n"
                              step' = (makeSteps'' net bound " :interpolation-group ga") ++ "\n\n"
                              initial = \k -> "(assert (! (= initial-" ++ (show k) ++ " " ++ (show (makeInit net k)) ++ ") :interpolation-group gb))"
                              initial' = foldr (\x y -> x ++ "\n\n" ++ y) "" (map (\x -> initial x) [0..bound-1])
                          in state ++ state' ++ state'' ++ step ++ step' ++ initial'


makeFormulas'' :: ColoredNetwork -> Int -> String
makeFormulas'' net bound = let state = foldr (\x y -> x ++ "\n\n" ++ y) "" (map (\x -> makeSignals' net x) [bound])
                               initial = \k -> "(assert (= initial-" ++ (show k) ++ " " ++ (show (makeInit net k)) ++ "))"
                               initial' = foldr (\x y -> x ++ "\n\n" ++ y) "" (map (\x -> initial x) [bound])
                           in state ++ initial'
...
makePrefInit' :: Int -> String
makePrefInit' bound = let r = "(assert (! (or " ++ (foldr (\x y -> if y /= "" then x ++ " " ++ y else x ++ y) "" (map (\x -> "initial-" ++ (show x) ++ "") [0..bound-1])) ++ ") :interpolation-group gb))"
                      in r


makeIplSteps' :: Int -> String
makeIplSteps' bound = let r = "(assert (! (and " ++ (foldr (\x y -> if y /= "" then x ++ " " ++ y else x ++ y) "" (map (\x -> "global_step-" ++ (show x) ++ "") [1..bound-1])) ++ ") :interpolation-group gb))"
                          r' = "(assert (! global_step-" ++ (show bound) ++ " :interpolation-group ga))"
                      in r ++ "\n\n" ++ r'


makeConsistentStates' :: Int -> String
makeConsistentStates' bound = let r = "(assert (! (and " ++ (foldr (\x y -> if y /= "" then x ++ " " ++ y else x ++ y) "" (map (\x -> "global_state-" ++ (show x) ++ "") [0..bound-2])) ++ ") :interpolation-group gb))"
                                  r' = "(assert (! global_state-" ++ (show (bound-1)) ++ " :interpolation-group ga :interpolation-group gb))"
                                  r'' = "(assert (! global_state-" ++ (show bound) ++ " :interpolation-group ga))"
                              in r ++ "\n\n" ++ r' ++ "\n\n" ++ r''


makeConsistentStates'' :: Int -> String
makeConsistentStates'' bound = let r = map (\x -> "(= global_state-" ++ (show x) ++ " true)") [bound]
                                   r' = foldr (\x y -> if y /= "" then x ++ " " ++ y else x ++ y) "" r
                               in "(assert " ++ r' ++ ")"


substituteVars :: Int -> String -> String
substituteVars bound x = replaceWithList [Replace (string'fromString $ "-" ++ (show (bound-1)) ++ ")") ("-" ++ (show bound) ++ ")"), Replace (string'fromString $ "-" ++ (show (bound-1)) ++ " ") ("-" ++ (show bound) ++ " ")] x


makeInitialApprox :: ColoredNetwork -> Map Literal Formula -> [Invariant Int] -> String -> Int -> String
makeInitialApprox net mlf invs lv bound = let (smtinvs, qs, v) = export_invariants_to_smt' net show_p invs
...
makeZeroRun :: ColoredNetwork -> Int -> String -> String
makeZeroRun net bound r = let vars = makeVars'' net bound
                              f = makeFormulas'' net bound
                              f' = makeConsistentStates'' bound
                              f'' = "(assert initial-" ++ show bound ++ ")"
                          in "(set-logic QF_LIA)" ++ "\n" ++ vars ++ "\n\n" ++ r ++ "\n\n" ++ f ++ "\n\n" ++ f' ++ "\n\n" ++ f'' ++ "\n\n(check-sat)\n\n(exit)"
...
makeKRun net bound r = let vars = makeVars net bound
                           ...
                           f''' = makeCheckInit bound
                       in "(set-logic QF_LIA)" ++ "\n" ++ vars ++ "\n\n" ++ r ++ "\n\n" ++ f ++ "\n\n" ++ f' ++ "\n\n" ++ f'' ++ "\n\n" ++ f''' ++ "\n\n(check-sat)\n\n(exit)"


checkFixpoint :: ColoredNetwork -> Int -> String -> String -> String -> String
checkFixpoint net bound r p p' = let vars = makeVars'' net bound
                                     f = makeFormulas''' net bound
                                     f' = makeConsistentStates'' bound
                                     f'' = "(assert (and " ++ p' ++ " (not " ++ p ++ ")))"
                                 in "(set-logic QF_LIA)" ++ "\n" ++ vars ++ "\n\n" ++ r ++ "\n\n" ++ f ++ "\n\n" ++ f' ++ "\n\n" ++ f'' ++ "\n\n(check-sat)\n\n(exit)"
